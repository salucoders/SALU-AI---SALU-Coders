...
export async function getActiveApiKey(): Promise<string> {
  // First attempt to get the API key from the backend configuration securely
  try {
    ...
    console.warn('Failed to fetch from /api/config, falling back to environment variables.');
  }
